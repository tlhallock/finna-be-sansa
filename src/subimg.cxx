...
typedef uint8_t unsigned1;
typedef int16_t signed2;
typedef int64_t signed8;
...
signed2* read_image(const char* filename, int& sx, int&sy);
 ...
 *   [-1:1]; November 2006 
 */
int search(const char *f1, const char *f2)
{
#if 0
  if (argc!=3)
    {
  ...
  int Asx, Asy;
  signed2* A = read_image(f1,Asx,Asy);
  int Asx2 = Asx/2+1;


  std::cout << "im1 has dim " << Asx << ", " << Asy << std::endl;
  
  int Bsx, Bsy;
  signed2* B = read_image(f2,Bsx,Bsy);

  std::cout << "im2 has dim " << Bsx << ", " << Bsy << std::endl;

  if (Bsx*Bsy > Asx*Asy)
  {
      std::cout << "The first image should be larger." << std::endl;
   ...
   * Aa is the amplitude of image A. Af is the frequency of image A
  ...
  fftw_plan backward = fftw_plan_dft_c2r_2d(Asy,Asx,Bf,crosscorrs,/*FFTW_FORWARD |*/ FFTW_ESTIMATE);
...
signed2* read_image(const char* filename, int& sx, int&sy)
{
  ...
  sx = input.width()  - (input.width() % 2);
  sy = input.height() - (input.height() % 2);
  assert(sx>4 && sy>4);
